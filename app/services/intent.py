...
INTENT_RULES = {
    "退换货": ["退货", "换货", "退掉", "换一下", "退款退货"],
    "售后": ["售后", "破损", "少件", "退款", "补发", "赔付", "质量问题"],
    "催发货": ["发货", "什么时候发", "怎么还没发", "催发货", "物流", "到货", "几天到"],
    "价格咨询": ["便宜", "优惠", "券", "折扣", "最低价", "补差", "保价", "多少钱"],
    "售前咨询": ["材质", "尺寸", "颜色", "适合", "怎么选", "有没有", "库存", "能不能", "是什么面料", "面料", "厚不厚"],
}
...
class IntentService:
    def recognize(self, request: IntentRecognizeRequest) -> IntentResult:
        ...
        if "退款" in text and any(word in text for word in ["破损", "少件", "质量", "售后"]):
            intent = "售后"
            score = max(score, 0.9)
